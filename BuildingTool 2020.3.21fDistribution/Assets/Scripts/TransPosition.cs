...
public class TransPosition : MonoBehaviour
{
    ...
    public Position TransLocalPosition(Vector3 position)
    {
        ...
        tempPosition.x = Mathf.FloorToInt((position.x / (10.0f / 10.0f))); // 1.0f;
        tempPosition.y = Mathf.FloorToInt((position.y / (10.0f / 10.0f)));
        tempPosition.z = Mathf.FloorToInt((position.z / (10.0f / 10.0f)));
